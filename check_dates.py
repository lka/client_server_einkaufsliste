...
print(f"Today: {today.strftime('%Y-%m-%d')} ({today.strftime('%A')}), weekday={today.weekday()}")
...
print(f"Next main shopping: {next_main_shopping.strftime('%Y-%m-%d')} ({next_main_shopping.strftime('%A')})")

print(f"\nFRESH_PRODUCTS_DAY={fresh_products_day} (Friday)")
print(f"Next fresh products: {next_fresh_products.strftime('%Y-%m-%d')} ({next_fresh_products.strftime('%A')})")
